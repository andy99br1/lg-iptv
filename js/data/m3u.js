...
// SAFE CACHE V4: known-working parser/player/EPG baseline + permanent URL-bound Live cache.
// Parses M3U playlists, separates Live TV / Movies / Series,
// and provides automatic XMLTV EPG support for M3U profiles.

const M3U_CACHE_KEY =
    "iptv_m3u_fixed_live_safe_v4";
...
function _m3uConfiguredUrlSync() {
    /*
     * Same sources used by the original m3uLoadConfig(), but synchronously.
     * This is only used to bind the persistent Live cache to the exact M3U URL.
     */
    try {
        const profiles =
            JSON.parse(
...
// ── Disk cache: Live only, permanent while URL is unchanged ──────────────────
//
// IMPORTANT:
// Keep the COMPLETE original channel objects here. The previous experimental
// compact cache rebuilt reduced objects and could disturb playback/EPG state.
// This version intentionally preserves the same object shape that already
// worked in the original 4-hour cache.
...
function m3uSaveCache(
    ...
    try {
        ...
        /*
         * _m3uLastLibrary is the known-working full parser result when this is
         * called after a normal load. Store only the small metadata EPG needs.
         */
        const epgUrl =
            (
                _m3uLastLibrary &&
        ...
        localStorage.setItem(
            ...
            JSON.stringify({
                ...
                /*
                 * Preserve full original objects — no compact conversion.
                 */
                channels:
                    channels,
        ...
        /*
         * Old caches cannot be safely matched to a URL, so discard them after
         * the new URL-bound cache has been written.
         */
        try {
        ...
        /*
         * Storage quota must never break IPTV playback.
         */
    }
}
...
function _m3uLibraryFromSafeLiveCache(
    cache,
    cfg
) {
    const channels =
    ...
    return {
        ...
        /*
         * These are deliberately empty. This object is only a Live/EPG cache
         * view. VOD/Series still call m3uGetLibrary(), which performs the full
         * known-working network parse in this session.
         */
        movies: [],
        movieCategories: [],
        series: [],
...
async function m3uFetchPlaylist(
    ...
    /*
     * app.js uses m3uFetchPlaylist(url) for its normal M3U refresh path.
     * With a fixed playlist URL, satisfy that refresh from the permanent cache.
     *
     * m3uGetLibrary() below passes forceNetwork=true so VOD/Series retain the
     * exact original full-library behavior.
     */
    if (!forceNetwork) {
        const cached =
            m3uLoadCache(
...
async function m3uGetChannelsAndCategories(
    ...
    /*
     * First load of a new URL: save the exact known-working channel objects.
     */
    m3uSaveCache(
        lib.channels,
        lib.categories
