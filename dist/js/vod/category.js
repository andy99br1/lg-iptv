...
function openCategory(title, type, catId) {
  ...
  fetchCached(ck, apiUrl('action=' + action + '&category_id=' + encodeURIComponent(catId))).then(function (data) {
...
function renderCategory(items, type) {
  ...
  document.getElementById('vod-category-count').textContent = items.length + (items.length === 1 ? ' title' : ' titles');
  var frag = document.createDocumentFragment();
  items.forEach(function (m) {
    frag.appendChild(makeCard(m, type));
  });
...
function categoryCards() {
  return Array.prototype.slice.call(elCategoryGrid.querySelectorAll('.vod-card'));
}
...
function categoryKey(kc) {
  ...
  if (kc === KEY.UP) {
    ...
    if (catFocus < 0) {
      ...
      catFocus = Math.min(cards.length - 1, catFocus + cols);
      paintCategoryFocus();
    }
  } else if (kc === KEY.LEFT) {
    if (catFocus > 0 && catFocus % cols !== 0) {
      catFocus--;
      paintCategoryFocus();
    }
  } else if (kc === KEY.RIGHT) {
    if (catFocus >= 0 && catFocus < cards.length - 1) {
      catFocus++;
      paintCategoryFocus();
    }
