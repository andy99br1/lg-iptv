...
window.Episodes = function () {
  ...
  function slimEpisodes(list) {
    ...
    for (var i = 0; i < list.length && i < 120; i++) {
      ...
      out.push({
        ...
        subs: typeof Subs !== 'undefined' ? Subs.fromEpisode(ep, _cfg()) : {
          files: [],
          embedded: [],
          unknown: 0
        }
  ...
  function resolveSeason() {
    if (_fetching) return;
    if (playlist()) {
      _why = 'season came with the metadata';
      return;
    }
    if (!_meta) {
    ...
    if (_meta.type !== 'episode') {
      _why = 'not an episode (type=' + (_meta.type || '?') + ')';
      return;
    }
    if (!_meta.series_id) {
      _why = 'metadata has no series_id, so the season cannot be looked up';
      return;
    }
    if (typeof Net === 'undefined' || typeof Config === 'undefined') {
      _why = 'core modules missing';
      return;
    }
    var cfg = _cfg();
    if (!cfg) {
      _why = 'no profile configured';
      return;
    }
    if (cfg.type === 'm3u') {
      _why = 'M3U source has no series API';
      return;
    }
    if (!cfg.server_url) {
      _why = 'profile has no server url';
      return;
    }
    _fetching = true;
    _why = 'looking up season ' + _meta.season + ' of series ' + _meta.series_id;
    Net.json(Config.apiUrl(cfg, 'action=get_series_info&series_id=' + encodeURIComponent(_meta.series_id)), {
      timeout: 12000
    }).then(function (data) {
      ...
      if (!list) {
        for (var k in eps) {
          ...
          for (var i = 0; i < eps[k].length; i++) {
            if (String(eps[k][i].id) === String(_meta.id)) {
              list = eps[k];
              break;
            }
          }
      ...
      if (!list) {
        ...
        _why = 'panel returned seasons [' + names.join(',') + '] — episode ' + _meta.id + ' is in none of them';
        settle();
        return;
      }
      ...
      for (var j = 0; j < pl.length; j++) {
        if (String(pl[j].id) === String(_meta.id)) {
          idx = j;
          break;
        }
      }
  ...
  function hasLearnedIntro() {
    return learnedIntroEnd() > 0;
  }
  ...
  function describe() {
    var pl = playlist(),
      i = index();
    var facts = 'meta=' + (_meta ? 'yes' : 'NO') + ' type=' + (_meta && _meta.type || '-') + ' sid=' + (_meta && _meta.series_id || '-') + ' season=' + (_meta && _meta.season || '-') + ' eps=' + (pl ? pl.length : 0) + ' idx=' + i;
    var verdict = hasNext() ? 'next=' + nextLabel() : _fetching ? 'looking up season...' : 'no next episode';
  ...
  return {
    ...
    hasLearnedIntro: hasLearnedIntro,
