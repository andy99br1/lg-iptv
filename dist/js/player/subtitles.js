...
window.PlayerSubs = function () {
  ...
  function hasFiles(r) {
    return !!(r && r.files && r.files.length);
  }
  ...
  function resolve() {
    ...
    if (_meta && _meta.type === 'movie' && _meta.id) {
      ...
      Net.json(Config.apiUrl(c, 'action=get_vod_info&vod_id=' + encodeURIComponent(_meta.id)), {
        ...
        _source = (r.embedded || []).length ? 'panel lists only embedded tracks' : 'panel lists no subtitles';
    ...
    if (_meta && _meta.type === 'episode' && typeof Episodes !== 'undefined' && Episodes.resolving && Episodes.resolving()) {
      _source = WAITING;
      return;
    }
  ...
  function probeSidecar(base) {
    ...
    Subs.probeSidecars(_url).then(function (found) {
      ...
      _probe = 'sidecar: ' + (found.length ? found.length + ' found' : 'none') + (log.length ? ' — ' + log.join('; ') : '');
      if (!found.length) {
  ...
  function describe() {
    ...
    for (var i = 0; i < emb && i < 4; i++) {
      var n = typeof Subs !== 'undefined' && Subs.langName(_found.embedded[i].lang) || _found.embedded[i].label;
      if (n && langs.indexOf(n) === -1) langs.push(n);
    }
    return 'files=' + files + ' embedded=' + emb + (langs.length ? ' [' + langs.join(', ') + ']' : '') + (_busy ? '  |  looking…' : '') + (_source ? '  |  ' + _source : '') + (
    ...
    _note ? '  |  handover: ' + _note : '') + (_probe ? '  |  ' + _probe : '') + (
