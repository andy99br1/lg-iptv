...
(function () {
  ...
  if (!rawMeta) {
    ...
    try {
      ...
      meta = null;
      metaNote = 'stored play info was corrupt';
    }
  }
  if (meta) {
    if (wantToken && meta.token) {
      if (meta.token !== wantToken) {
        metaNote = 'stored play info belongs to a different play (token ' + meta.token + ' vs ' + wantToken + ')';
        meta = null;
      }
      ...
      metaNote = 'stored url does not match the one opened' + '  stored=' + _tail(meta.url) + '  opened=' + _tail(url);
      meta = null;
    }
  }
  ...
  function startEpisode(pl, idx, base) {
    ...
    window.location.href = 'player.html?t=' + encodeURIComponent(token) + '&url=' + encodeURIComponent(nextUrl) + '&title=' + encodeURIComponent(name);
  }
  ...
  function firePill() {
    ...
    cancelAutoNext();
    hidePill();
    if (a === 'intro') Episodes.skipIntro();else if (a === 'next') Episodes.playNext();
  }
  ...
  function refreshPill() {
    ...
    if (Episodes.introVisible()) {
      showPill('Skip intro  ·  OK', 'intro');
      return;
    }
    if (Episodes.outroVisible()) {
      showPill('Next episode  ·  OK', 'next');
      return;
    }
    hidePill();
  }
  ...
  function pillVisible() {
    return !!(pillEl && pillEl.className === 'pill-visible' && pillAction);
  }
  ...
  var controls = ['ctrl-rewind', 'ctrl-play', 'ctrl-forward', 'ctrl-next', 'ctrl-audio', 'ctrl-subs', 'ctrl-speed', 'ctrl-fullscreen'].map(function (id) {
  ...
  function recallTrack(what) {
    return _recalledTrack(TRACK_KEY, what);
  }
  function rememberTrack(what, val) {
    _rememberTrack(TRACK_KEY, what, val);
  }
  var menuMode = 'subs'; // 'audio' | 'subs' | 'os' — drives BACK behaviour

  function isSelectable(opt) {
    return opt && (opt.kind === 'track' || opt.kind === 'audio' || opt.kind === 'action' || opt.kind === 'os' || opt.kind === 'stepper');
  }
  ...
  function cycleIndex(list, current, dir) {
    var i = 0;
    for (var n = 0; n < list.length; n++) if (list[n][0] === current) {
      i = n;
      break;
    }
    i += dir;
    ...
    if (audio.length) {
      audio.forEach(function (t) {
        subsOptions.push({
          ...
          label: t.label,
          track: t,
          current: t.id === activeAudio
        });
      });
    } else {
      subsOptions.push({
        kind: 'note',
        label: window.player && player.audioUnavailableReason ? player.audioUnavailableReason() : 'No alternative audio tracks.'
      });
    }
  }
  function buildSubsOptions() {
    ...
    if (!subs.length) {
      ...
      subsOptions.push({
        kind: 'note',
        label: window.player && player.subsUnavailableReason ? player.subsUnavailableReason() : 'No subtitles found for this title.'
      });
    }
    subsOptions.push({
      kind: 'track',
    ...
    if (searchProviders().length) {
      ...
      if (typeof OpenSubtitles === 'undefined' || !OpenSubtitles.configured()) {
        subsOptions.push({
          kind: 'note',
          label: 'Searching Assrt, which is strongest on Chinese subtitles. ' + 'For English, add a free OpenSubtitles API key in Settings → Subtitles ' + 'and both are searched together.'
        });
      }
    }
    subsOptions.push({
    ...
    subsOptions.push(stepper('Delay', function () {
      return fmtDelay(player.getSubDelay());
    }, function (dir) {
      player.setSubDelay(player.getSubDelay() + dir * 0.25);
    }, {
    ...
    subsOptions.push(stepper('Position', function () {
      return SUB_POS_NAMES[player.getSubPosition()] || 'Default';
    }, function (dir) {
      player.setSubPosition(player.getSubPosition() + dir);
    }, {
      reset: function reset() {
        player.setSubPosition(0);
      }
    }));
    subsOptions.push(stepper('Text size', function () {
      var v = player.getSubSize();
      for (var i = 0; i < SUB_SIZES.length; i++) if (SUB_SIZES[i][0] === v) return SUB_SIZES[i][1];
      return 'Normal';
    }, function (dir) {
      player.applySubSize(SUB_SIZES[cycleIndex(SUB_SIZES, player.getSubSize(), dir)][0]);
    }));
    subsOptions.push(stepper('Style', function () {
      var v = player.getSubStyle();
      for (var i = 0; i < SUB_STYLES.length; i++) if (SUB_STYLES[i][0] === v) return SUB_STYLES[i][1];
      return 'Shadow';
    }, function (dir) {
      player.applySubStyle(SUB_STYLES[cycleIndex(SUB_STYLES, player.getSubStyle(), dir)][0]);
    }));
    subsOptions.push(stepper('Colour', function () {
      var v = player.getSubColour();
      for (var i = 0; i < SUB_COLOURS.length; i++) if (SUB_COLOURS[i][0] === v) return SUB_COLOURS[i][1];
      return 'White';
    }, function (dir) {
      player.applySubColour(SUB_COLOURS[cycleIndex(SUB_COLOURS, player.getSubColour(), dir)][0]);
    }));
  }
  ...
  function buildSpeedOptions() {
    ...
    if (typeof Episodes !== 'undefined' && Episodes.hasLearnedIntro()) {
      ...
      subsOptions.push({
        kind: 'action',
        label: '↺  Forget this series’ intro skip',
        action: 'forget-intro'
      });
      subsOptions.push({
        kind: 'note',
        label: 'Skip intro currently jumps to a point you chose earlier. Forget it to go back to the default jump.'
      });
      subsOptions.push({
        kind: 'header',
        label: 'Speed',
    ...
    if (window.player && player.speedSupported && !player.speedSupported()) {
      subsOptions.push({
        kind: 'note',
        label: "This TV's player doesn't support changing playback speed."
      });
      return;
    }
    subsOptions.push(stepper('Speed', function () {
      var r = player.getSpeed();
      return (r === 1 ? '1' : String(r)) + '×';
    }, function (dir) {
      var r = player.getSpeed(),
        i = 0;
      for (var n = 0; n < SPEEDS.length; n++) if (Math.abs(SPEEDS[n] - r) < 0.01) {
        i = n;
        break;
      }
      ...
      hint: 'OK resets to normal speed.'
    }));
  }
  ...
  function rankResults(rows, providers) {
    ...
    return rows.sort(function (a, b) {
      ...
      var da = Number(a.downloads) || 0,
        db = Number(b.downloads) || 0;
  ...
  function openOsSearch() {
    ...
    if (!q.title && !q.imdbId && !q.tmdbId) {
      subsOptions = [{
        kind: 'note',
        label: 'Nothing to search for — this title has no name.'
      }];
      renderMenu('Online subtitles');
      return;
    }
    searchAll(q).then(function (results) {
      ...
      if (!results.length) {
        ...
        subsOptions = results.slice(0, 20).map(function (r) {
          ...
          return {
            ...
            sub: (r.providerLabel ? r.providerLabel + '  ·  ' : '') + (matched ? r.release + '  ·  ' : '') + extra.join(' · ')
          };
        });
      }
      renderMenu('Online subtitles');
      ...
      subsOptions = [{
        kind: 'note',
        label: err && err.message ? err.message : 'Search failed.'
      }];
      renderMenu('Online subtitles');
    });
  }
    ...
    api.download(opt.fileId, opt.lang).then(function (res) {
      ...
      waitForTrack(_osLabel(opt), function (found) {
        if (found && player.setSubtitle) {
          ...
          subsOptions = [{
            kind: 'note',
            label: 'Downloaded, but the subtitle file could not be read.'
          }];
          renderMenu('Online subtitles');
        }
      });
      ...
      subsOptions = [{
        kind: 'note',
        label: err && err.message ? err.message : 'Download failed.'
      }];
      renderMenu('Online subtitles');
    });
  }
  ...
  function waitForTrack(label, done) {
    ...
    (function poll() {
      var subs = window.player && player.listSubtitles ? player.listSubtitles() : [];
      for (var i = 0; i < subs.length; i++) if (subs[i].label === label) {
        done(true);
        return;
      }
      if (++tries > 30) {
  ...
  function menuTitleFor(mode) {
    return mode === 'audio' ? 'Audio' : mode === 'speed' ? 'Playback' : mode === 'os' ? 'Online subtitles' : 'Subtitles';
  }
  function applyTrackChoice() {
    ...
    if (opt.kind === 'action' && opt.action === 'os') {
      openOsSearch();
      return;
    }
  ...
  video.addEventListener('loadeddata', function () {
    setTimeout(restoreTrackPreferences, 400);
  });
  ...
  if (window.player) {
    player.onTracksChanged = function () {
      ...
      if (subsOpen && menuMode === 'subs') {
        buildSubsOptions();
        renderMenu('Subtitles');
      } else if (subsOpen && menuMode === 'audio') {
        buildAudioOptions();
        renderMenu('Audio');
      }
    };
