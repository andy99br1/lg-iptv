...
window.Assrt = function () {
  ...
  function buildQuery(q) {
    ...
    if (q.season || q.episode) {
      if (q.season != null && q.season !== '') {
        parts.push(q.episode != null && q.episode !== '' ? 'S' + pad2(Number(q.season)) + 'E' + pad2(Number(q.episode)) : 'S' + pad2(Number(q.season)));
      }
    } else if (q.year) {
  ...
  function search(q) {
    ...
    var url = BASE + '/sub/search?token=' + encodeURIComponent(token()) + '&cnt=15&q=' + encodeURIComponent(query);
    return Net.json(url, {
      timeout: TIMEOUT
    }).then(function (body) {
  ...
  function download(fileId, lang) {
    var url = BASE + '/sub/detail?token=' + encodeURIComponent(token()) + '&id=' + encodeURIComponent(fileId);
    return Net.json(url, {
      timeout: TIMEOUT
    }).then(function (body) {
  ...
  function statusError(body) {
    ...
    var msg = STATUS_TEXT[code] || (body && body.errmsg ? 'Assrt: ' + body.errmsg : 'Assrt refused the request (status ' + code + ').');
