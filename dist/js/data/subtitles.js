...
window.Subs = function () {
  ...
  function langFrom(obj) {
    return obj.lang || obj.language || obj.tags && (obj.tags.language || obj.tags.LANGUAGE) || '';
  }
  function titleFrom(obj) {
    return obj.title || obj.name || obj.label || obj.tags && (obj.tags.title || obj.tags.TITLE) || '';
  }
  ...
  function parse(raw, cfg) {
    ...
    if (!raw) return out;
    var list;
    if (typeof raw === 'string') list = unwrapString(raw);else if (isArray(raw)) list = raw;else if (_typeof(raw) === 'object') {
      /* Language-keyed map: { eng: "…", fre: {…} } */
      list = [];
      for (var k in raw) {
        if (!Object.prototype.hasOwnProperty.call(raw, k)) continue;
        var v = raw[k];
        if (typeof v === 'string') list.push({
          url: v,
          lang: k
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      if (typeof s === 'string') {
        var u = String(s).trim();
        if (u) out.files.push(makeFile(u, '', '', cfg));else out.unknown++;
        continue;
      }
  ...
  function fromVodInfo(data, cfg) {
    return merge([parse(pick(data && data.info), cfg), parse(pick(data && data.movie_data), cfg), parse(pick(data), cfg)]);
  }
  function fromEpisode(ep, cfg) {
    return merge([parse(pick(ep && ep.info), cfg), parse(pick(ep), cfg)]);
  }
  ...
  function describePayload(data) {
    ...
    if (_typeof(data) !== 'object') return 'reply was ' + _typeof(data);
    var parts = [];
  ...
  function subKeyReport(o) {
    var keys = ['subtitles', 'subtitle', 'subs', 'sub', 'subtitle_tracks', 'text_tracks', 'streams'];
    var out = [];
  ...
  function shapeOf(v, isStreams) {
    ...
    if (_typeof(v) === 'object') return 'object{' + ownKeys(v).slice(0, 6).join(',') + '}';
    return String(_typeof(v));
  }
  function ownKeys(o) {
    var out = [];
  ...
  function rawSample(data, max) {
    ...
    try {
      ...
      return 'reply could not be read';
    }
    if (!json) return 'reply was empty';
    return json.length > max ? json.slice(0, max) + '…(+' + (json.length - max) + ' more)' : json;
  }
  ...
  var PICK_KEYS = ['subtitles', 'subtitle', 'subs', 'sub', 'subtitle_tracks', 'text_tracks'];
  function pick(o) {
    if (!o) return null;
  ...
  function looksLikeSubtitle(text) {
    ...
    if (/\[Script Info\]/i.test(head) || /\[(?:V4\+?|V4\+\+) Styles\]/i.test(head) && /\[Events\]/i.test(head)) return true;
  ...
  function probeOne(url) {
    ...
    return Net.request(url, {
      ...
      if (!partial && (len === 0 || len > PROBE_MAX)) {
        _probeLog.push(name + ': ' + (len ? 'too big (' + Math.round(len / 1024) + 'KB)' : 'no size given and range ignored'));
        return null;
      }
      return res.text();
    }).then(function (text) {
      ...
      if (looksLikeSubtitle(text)) {
        _probeLog.push(name + ': subtitle found');
        return url;
      }
  ...
  function describe(result, opts) {
    ...
    /* The suggestion has to match what the menu actually offers. Telling
       ...
       there. */
    var fallback = opts.hasOpenSubtitles ? 'use “Search online…” below' : 'use “Search online…” below (and add a free OpenSubtitles API key in ' + 'Settings → Subtitles for better English results)';
    if (r.embedded.length) {
      ...
      var msg = 'Your provider lists ' + r.embedded.length + ' subtitle track' + (r.embedded.length === 1 ? '' : 's') + ' (' + list + ') built into this video, ' + 'not as separate files.';
      if (opts.hasHlsTier) {
        msg += ' Press RED to try the HLS player, which reads tracks in software — ' + 'otherwise ' + fallback + '.';
      } else {
    ...
    if (r.failed && r.requested > 0 && r.failed >= r.requested) {
      return 'Your provider listed ' + r.requested + ' subtitle file' + (r.requested === 1 ? '' : 's') + ' for this title, but ' + (r.requested === 1 ? 'it' : 'they') + " couldn't be downloaded — " + fallback + '.';
    }
    if (r.unknown) {
      return 'Your provider returned ' + r.unknown + ' subtitle entr' + (r.unknown === 1 ? 'y' : 'ies') + ' this app could not read — ' + fallback + '.';
    }
    return 'Your provider supplies no subtitles for this title. ' + 'Most embed them in the video instead — ' + fallback + '.';
  }
  ...
  function toVtt(text) {
    ...
    s = s
    /* Drop the numeric cue index on its own line. */.replace(/^\d+\s*$/gm, '')
    /* MM:SS,mmm → 00:MM:SS.mmm  (both ends of the arrow, hence twice). */.replace(/(^|\s)(\d{1,2}:\d{2}[,.]\d{1,3})(\s*-->)/g, function (_, pre, t, post) {
      return pre + '00:' + t.replace(',', '.') + post;
    }).replace(/(-->\s*)(\d{1,2}:\d{2}[,.]\d{1,3})(\s|$)/g, function (_, pre, t, post) {
      return pre + '00:' + t.replace(',', '.') + post;
    })
    /* HH:MM:SS,mmm → HH:MM:SS.mmm */.replace(/(\d{1,2}:\d{2}:\d{2}),(\d{1,3})/g, '$1.$2').replace(/\n{3,}/g, '\n\n').trim();
    return 'WEBVTT\n\n' + s;
  }
  return {
    DIAG_KEY: DIAG_KEY,
